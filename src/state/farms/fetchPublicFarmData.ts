...
type PublicFarmData = {
  ...
  depositFee: string
  withdrawFee: string
  dcxaPerBlock: SerializedBigNumber
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, dcxaPerBlock] =
    pid || pid === 0
  ...
  const AmaPerBlock = dcxaPerBlock ? new BigNumber(dcxaPerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    dcxaPerBlock: AmaPerBlock.toJSON(),
  }
}
